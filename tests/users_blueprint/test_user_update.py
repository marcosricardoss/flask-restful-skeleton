...
def test_user_update_with_data_well_formatted_returning_200_status_code(client):
...
def test_user_update_without_data_returning_400_status_code(client):
    user = create_user()
    endpoint = '/users/{}'.format(user.id)
    response = client.put(endpoint, content_type='application/json')
...
def test_user_update_with_empty_data_returning_400_status_code(client):
    user = create_user()
    endpoint = '/users/{}'.format(user.id)
    data = {}
    response = client.put(endpoint,
...
def test_user_update_without_username_returning_400_status_code(client):
    user = create_user()
    endpoint = '/users/{}'.format(user.id)
    data = {'password': 'x123x'}
    response = client.put(endpoint,
...
def test_user_update_without_password_returning_400_status_code(client):
    user = create_user()
    endpoint = '/users/{}'.format(user.id)
    data = {'username': 'user'}
    response = client.put(endpoint,
...
def test_user_update_with_a_existent_username_returning_400_status_code(client):
    user1 = create_user()
    user2 = create_user()
