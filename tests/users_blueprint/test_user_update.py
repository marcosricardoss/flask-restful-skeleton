...
def test_user_update_with_empty_data_returning_400_status_code(client, session, auth):
    user = create_user(session)
    endpoint = '/users/{}'.format(user.id)
    data = {}
    response = client.put(endpoint,
...
def test_user_update_without_username_returning_400_status_code(client, session, auth):
    user = create_user(session)
    endpoint = '/users/{}'.format(user.id)
    data = {'password': 'x123x'}
    response = client.put(endpoint,
...
def test_user_update_without_password_returning_400_status_code(client, session, auth):
    user = create_user(session)
    endpoint = '/users/{}'.format(user.id)
    data = {'username': 'user'}
    response = client.put(endpoint,
...
def test_user_update_with_a_existent_username_returning_400_status_code(client, session, auth):
