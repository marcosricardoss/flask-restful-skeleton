...
def test_user_register_with_empty_data_returning_400_status_code(client, auth):
    data = {}
    response = client.post('/users',
                           data=json.dumps(data),
                           content_type='application/json',
...
def test_user_register_without_username_returning_400_status_code(client, auth):
    data = {'password': "123"}
    response = client.post('/users',
                           data=json.dumps(data),
                           content_type='application/json',
...
def test_user_register_without_password_returning_400_status_code(client, auth):
...
def test_user_register_with_a_existent_username_returning_400_status_code(client, session, auth):
