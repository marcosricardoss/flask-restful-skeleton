...
def test_user_retrieve_with_existent_user_id_returning_200_status_code(client, session):
...
def test_user_retrieve_with_existent_username_returning_200_status_code(client, session):
    user = create_user(session)
...
def test_user_retrieve_user_list_200_status_code(client, session):    
...
def test_user_retrieve_with_inexistent_user_id_returning_404_status_code(client):
    endpoint = '/users/{}'.format(get_unique_id)
    response = client.get(endpoint)
    assert response.status_code == 404


def test_user_retrieve_with_inexistent_username_returning_404_status_code(client):
